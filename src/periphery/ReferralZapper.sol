...
contract ReferralZapper {
    ...
    /// @notice Referral deposit wrapper address
    ...
    constructor() {
        BOLD.forceApprove(address(REFERRAL_DEPOSIT_WRAPPER), type(uint256).max);
        YEARN_BOLD.forceApprove(address(REFERRAL_DEPOSIT_WRAPPER), type(uint256).max);
    }
    ...
    /// @param _referrer The address that referred the depositor
    ...
    function zapIn(
        ...
        // BOLD --> yBOLD
        uint256 _shares =
            REFERRAL_DEPOSIT_WRAPPER.depositWithReferral(address(YEARN_BOLD), _assets, address(this), _referrer);

        // yBOLD --> st-yBOLD
        return REFERRAL_DEPOSIT_WRAPPER.depositWithReferral(address(STAKED_YEARN_BOLD), _shares, _receiver, _referrer);
    }
